    ...
    <div class="row">
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">1. Curso | 2. Sesiones | 3. Mentores | <b>4. Mediadores</b> | 5. Alumnos
                </div>
                <div class="panel-body">
                    <div class="ibox float-e-margins">
                        <a href="{{ route('grade.teachers', $grade->id) }}" title="Atrás">
                            <button class="btn btn-warning btn-xs"><i class="fa fa-arrow-left" aria-hidden="true"></i>
                                Atrás
                            </button>
                        </a>
                        <a href="{{ route('grade.students', $grade->id) }}" title="Atrás">
                                                ...
                                                <tr>
                                                    ...
                                                    @if($item->division_users->where('grade_id', $grade->id)->count() == 0)
                                                        <form class="form-add-user-grade" method="POST" action="{{ route('division-user.store') }}" accept-charset="UTF-8" style="display:inline" id="add_user-{{ $item->id }}">
                                                            ...
                                                            <button type="submit" class="btn btn-primary btn-xs" title="Vincular Voluntario">
                                                        ...
                                                        <form class="form-del-user-grade" method="POST" action="{{ route('grade.delete.user') }}" accept-charset="UTF-8" style="display:none" id="del_user-{{ $item->id }}">
                                                            ...
                                                            <input type="hidden" name="user_id" value="{{ $item->id }}">
                                                            <input type="hidden" name="rol_id" value="{{ $rol }}">
                                                            <button type="submit" class="btn btn-danger btn-xs" title="Quitar Voluntario">
