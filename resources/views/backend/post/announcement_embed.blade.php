<div class="table-responsive">
    <a href="{{ route('grade.index', $grade->id) }}" title="Atrás">
    ...
    @isset($type_forum_grade)
        ...
        <br>
        <div>
            <a title="Crear nueva discusión" aria-hidden="true" href=" {{ route('post.create',"notice") }}" class="btn btn-primary btn-md">Crear nuevo tema</a>
        </div>
        <br>
        @endif
        @endroles
            ...
            @for($i = 0;$i < count($announcements);$i++)
                <tr>
                    <td class="border-right">
                        <a href="{{route('post.show',$announcements[$i]->id) }}"
                    ...
                    <td class="border-right">
                        {{ $count_comments[$cont_comments]->ChildCount }}
                    </td>
                    @if($cont_last_comment < count($lasted_comments))
                        <td align="right" class="border-right">
                            {{ $lasted_comments[$cont_last_comment]->firstname." "
                            .$lasted_comments[$cont_last_comment]->lastname  }}
                            <br>
